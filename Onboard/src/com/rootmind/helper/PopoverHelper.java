...
public class PopoverHelper extends Helper {
	...
	public AbstractWrapper fetchPopoverData(PopoverWrapper popoverWrapper)throws Exception {
		...
		try {
			...
			System.out.println("Popover info data fetch completed " );
...
public AbstractWrapper deletePopoverData(PopoverWrapper popoverWrapper)throws Exception {
		...
		try {
			con = getConnection();
				...
				System.out.println("Popover delete table "+popoverWrapper.tableName);
				
				PreparedStatement pstmt = con.prepareStatement("DELETE FROM [" + popoverWrapper.tableName + "] WHERE Desc=?");
				...
				System.out.println("Successfully " +popoverWrapper.desc+" Deleted");
			...
			try {
				...
				for(int i=0;i<=popoverWrapper.length-1;i++){
					...
					//System.out.println("popoverWrapper.length " + popoverWrapper.length);
							...
							System.out.println("in try Table name is " + popoverWrapper[i].tableName);
				...
				if (vector.size()>0)
				{
					...
					dataArrayWrapper.popoverWrapper = new PopoverWrapper[0];
					dataArrayWrapper.popoverWrapper=popoverWrapper;
					dataArrayWrapper.recordFound=true;
				}
	...
	public List<PopoverWrapper> fetchPopoverDataList(PopoverWrapper popoverWrapper,String tableName)throws Exception {
		
		
			List<PopoverWrapper> listPopoverWrapper = null;
		...
		try {
			...
			DataArrayWrapper dataArrayWrapper=(DataArrayWrapper)fetchPopoverData(popoverWrapper);	
			
			listPopoverWrapper = Arrays.asList(dataArrayWrapper.popoverWrapper);
	...
	public String fetchPopoverDesc(String code,String tableName)throws Exception {
		...
		try {
				...
				System.out.println(" fetchPopoverDesc try Table name is " + tableName);
	...
	public AbstractWrapper fetchTableNames(PopoverWrapper popoverWrapper)throws Exception {
		...
		try {
			...
			sql="SELECT Code, Description, Filter FROM RMT_TableNames";
